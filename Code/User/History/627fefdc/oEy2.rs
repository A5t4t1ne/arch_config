...
fn main() {
    ...
    let transformed_numbers: Vec<i32> = numbers.into_iter()
        .map(|x: i32| x*3)
        .collect();
    println!("{:?}", transformed_numbers);
    ...
    let words: Vec<&str> = vec!["autobot", "beach", "car", "decepticon", "energon", "frothy"];
    let transformed: Vec<String>  = words.into_iter().filter(|x| !x.contains('h')).map(|y: &str| (*y).to_uppercase()).collect();
