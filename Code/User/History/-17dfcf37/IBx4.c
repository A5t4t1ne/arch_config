...
static void get_n_chars(char *dest, size_t n, char *prompt, bool hidden);
...
int main()
{
    ...
    while (running)
    {
        ...
        case 'e':
        {
            ...
            get_n_chars(key, MAX_KEY_LEN, "Enter the key: ", false);
            if (key[0] == '\0')
            {
                printf("\033[31m"); // set text color to red
            ...
            while (!inp_valid)
            {
                ...
                {
                case '1':
                    get_n_chars(val, MAX_VAL_LEN, "Enter the new value: ", false);
                    inp_valid = true;
                    break;
                case '2':
                {
                    ...
                    get_n_chars(len_inp, 3, "Enter length of password: ", false);
...
static void get_db_name(unsigned char *name)
{
    ...
    while (!name_valid)
    {
        get_n_chars(name, MAX_DB_NAME_LEN, "Enter DB name: ", false);
 ...
 * @param hidden if true, the input will be hidden
 */
static void get_n_chars(char *dest, size_t n, char *prompt, bool hidden)
{
    ...
    while (true)
    {
        ...
        {
            printf("\033[31m"); // set text color to red
            printf("[!] Input too long, max length: %ld\n", n);
            printf("\033[0m"); // reset text color to default
        }
        else
        {
...
static void request_master_password(unsigned char *output, bool new)
{
    ...
    while (!pw_accepted)
    {
        ...
        get_n_chars(buf, MAX_MASTER_PW_LEN, prompt, true);
        ...
        if (new)
        {
            ...
            get_n_chars(buf2, MAX_MASTER_PW_LEN, prompt, true);
