...
def reverse_hash(hash_value):
    ...
    # Undo the final XOR operation
    intermediate_hash = ctypes.c_ulonglong(hash_value ^ xor_value).value
    ...
    # Undo the XOR operation for the last byte
    last_byte = int(intermediate_hash)
    if last_byte > 255:
        last_byte = 255
    bytes_list.append(last_byte)

    # Reconstruct the original string
    original_string = b"".join(bytes(bytearray(reversed(bytes_list)))).decode(
        "utf-8", "replace"
    )
    return original_string
...
for tool in known_debug_tools:
    hash_value = get_hash(tool)
    if hash_value in assemblyTimeStamps:
        print(f"{tool}: {hash_value}")
