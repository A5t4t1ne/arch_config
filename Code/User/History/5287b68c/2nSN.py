...
class Parser:
    @staticmethod
    def parse_text(fname: str) -> Maze:
        with open(fname, 'r') as f:
            maze_fstr = f.readlines()

        maze: list[list[Node]] = []
        start = None
        goal = None
        for y, row in enumerate(maze_fstr):
            maze_row = []
            for x, symbol in enumerate(row):
                ...
                neighbours = []
                if x > 0:
                    neighbours.append(maze_row[x-1])
                if y > 0:
                    neighbours.append(maze[y-1][x])
                    
                maze_row[-1].neighbors = neighbours
