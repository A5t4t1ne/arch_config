from enum import Enum
from collections import namedtuple

# create ENUM for different node types
class NodeType(Enum):
    WALL = '#'
    OPEN = '.'
    START = 'A'
    GOAL = 'B'


class Node:
    def __init__(self, type: NodeType, x, y, neighbours: list = []) -> None:
        self.type = type
        self.symbol = type.value
        self.neighbors = neighbours
        Pos = namedtuple('Pos', ['x', 'y'])
        self.pos = Pos(x, y)


class Maze:
    def __init__(self, maze: list[Node], start: Node, goal: Node) -> None:
        self.maze = maze
        self.start = start
        self.goal = goal

    def print_maze(self):
        for row in self.maze:
            print(row)


class Parser:
    def __init__(self, fname) -> None:
        with open(fname) as f:
            self.maze = f.read().splitlines()

    def parse_text(self):
        maze = []
        for y, row in enumerate(self.maze):
            maze_row = []
            for x, symbol in enumerate(row):
                if symbol == NodeType.WALL.value:
                    maze_row.append(Node(NodeType.WALL))
                elif symbol == NodeType.OPEN.value:
                    maze_row.append(Node(NodeType.OPEN))
                elif symbol == NodeType.START.value:
                    start = Node(NodeType.START)
                    maze_row.append(start)
                elif symbol == NodeType.GOAL.value:
                    goal = Node(NodeType.GOAL)
                    maze_row.append(goal)
            maze.append(maze_row)
        return Maze(maze)



