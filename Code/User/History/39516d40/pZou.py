    ...
    for f in files:
        if f in data:
            ...
            if len(missmatched_attr) > 0:
                errors['missmatch'] = errors.get('missmatch', 0) + 1
                missmatch(f, data, missmatched_attr)
            missmatched_attr.clear()
        else:
            print(f'[WARNING] new file: {f}')
            errors['new_file'] = errors.get('new_file', 0) + 1

    for d in directories:
        if d not in data:
            print(f'[WARNING] new directory: {d}')
            errors['new_dir'] = errors.get('new_dir', 0) + 1
            
    return  errors['missmatch'] == 0 and \
            errors['new_file'] == 0 and \
            errors['new_dir'] == 0
