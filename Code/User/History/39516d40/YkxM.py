...
def verify(data, files, directories):
    ...
    for f in files:
        if f in data:
            ...
            data['last_it_file_err']['new_file'] = data['last_it_file_err'].get('new_file', 0) + 1

    for d in directories:
        if d not in data:
            print(f'[WARNING] new directory: {d}')
            data['last_it_file_err']['new_dir'] = data['last_it_file_err'].get('new_dir', 0) + 1
            
    return  data['last_it_file_err']['missmatch'] == 0 and \
            data['last_it_file_err']['new_file'] == 0 and \
            data['last_it_file_err']['new_dir'] == 0
...
def missmatch(path: str, data, missmatched_attr: dict):
    ...
    data['last_it_file_err']['missmatch'] = data['last_it_file_err'].get('missmatch', 0) + 1
