...
void move_col_sel_cursor(int col)
{
    ...
    if (scanf("\033[%d;%dR", &end_pos_row, &end_pos_col) != 2) {
        // Error occurred
    }
    
    printf("\033[s"); // Save the cursor position for resetting it

    // int y = 4 * col + COL_OFFSET;
    // int x = ROW_OFFSET - 2;
    // int y_ahead = 4 * ((col + 1) % COLS) + COL_OFFSET;
    // int y_behind = 4 * ((col - 1 + COLS) % COLS) + COL_OFFSET;
    int y = end_pos_row - ROWS - 1;
    int x = end_pos_col - COLS - 1;
    int y_ahead = (y + 4) % COLS;
    int y_behind = (y - 4) % COLS;
